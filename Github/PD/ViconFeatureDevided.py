...
def extract(input_file,straight_file,turn_file,afterturn_file):
    ...
    #print df.head()
    #fill all theNA as 0
    #df.fillna(method='ffill').fillna(method='bfill')   ##fill forwarding and backwarding
    print df
    ...
    df['acToeLX'] = df['spToeLX'].diff(-2)/(timeInterval*2)    #-1 means the period=-1, the latter-former
    df['acToeLY'] = df['spToeLY'].diff(-2)/(timeInterval*2)
    df['acToeLZ'] = df['spToeLZ'].diff(-2)/(timeInterval*2)
    df['acHeelLX'] = df['spHeelLX'].diff(-2)/(timeInterval*2)
    df['acHeelLY'] = df['spHeelLY'].diff(-2)/(timeInterval*2)
    df['acHeelLZ'] = df['spHeelLZ'].diff(-2)/(timeInterval*2)
    df['acHeelRX'] = df['spHeelRX'].diff(-2)/(timeInterval*2)
    df['acHeelRY']  = df['spHeelRY'].diff(-2)/(timeInterval*2)
    df['acHeelRZ'] = df['spHeelRZ'].diff(-2)/(timeInterval*2)
    df['acToeRX'] = df['spToeRX'].diff(-2)/(timeInterval*2)
    df['acToeRY'] = df['spToeRY'].diff(-2)/(timeInterval*2)
    df['acToeRZ'] = df['spToeRZ'].diff(-2)/(timeInterval*2)
    df['ackneeLX'] = df['spkneeLX'].diff(-2)/(timeInterval*2)
    df['ackneeLY'] = df['spkneeLY'].diff(-2)/(timeInterval*2)
    df['ackneeLZ'] = df['spkneeLZ'].diff(-2)/(timeInterval*2)
    df['ackneeRX'] = df['spkneeRX'].diff(-2)/(timeInterval*2)
    df['ackneeRY']= df['spkneeRY'].diff(-2)/(timeInterval*2)
    df['ackneeRZ'] = df['spkneeRZ'].diff(-2)/(timeInterval*2)
    df['acshRX'] =  df['spshRX'].diff(-2)/(timeInterval*2)
    df['acshRY']= df['spshRY'].diff(-2)/(timeInterval*2)
    df['acshRZ'] = df['spshRZ'].diff(-2)/(timeInterval*2)
    df['acshLX'] = df['spshLX'].diff(-2)/(timeInterval*2)
    df['acshLY'] = df['spshLY'].diff(-2)/(timeInterval*2)
    df['acshLZ'] = df['spshLZ'].diff(-2)/timeInterval*2
    ...
    '''
    To seperate the three phase of walking
    Using threshold:
    |ToeX|  > 500 && |ToeY| < 500   straight walking
    |ToeX|  > 500 && |ToeY| >500   turning around phase
    |ToeX|  < 500 && |ToeY| < 500   after turn around phase
    ...
    #turn 
    turnStartDF1 = df.loc[(df['spHeelLX'].abs()>thretholdforTurn) & (df['spHeelLY'].abs()>thretholdforTurn)].head(1)
    turnStartDF2 = df.loc[(df['spHeelRX'].abs()>thretholdforTurn) & (df['spHeelRY'].abs()>thretholdforTurn)].head(1)
    startIndex = min(int(turnStartDF1.index.get_values()),int(turnStartDF2.index.get_values()))
    print startIndex
       
    turnEndDF1 = df.loc[(df['spHeelLX'].abs()>thretholdforTurn) & (df['spHeelLY'].abs()>thretholdforTurn)].tail(1)
    turnEndDF2 = df.loc[(df['spHeelRX'].abs()>thretholdforTurn) & (df['spHeelRY'].abs()>thretholdforTurn)].tail(1)
    endIndex = max(int(turnEndDF1.index.get_values()),int(turnEndDF2.index.get_values()))
    print endIndex   
    
    with open(straight_file, 'a') as f:
        df[:startIndex+25].to_csv(f,sep=',',header=True)
        #print df[:startIndex]
        
    with open(turn_file, 'a') as f:
        df[startIndex-25:endIndex+25].to_csv(f,sep=',',header=True)
        
    with open(afterturn_file, 'a') as f:
        df[endIndex-25:].to_csv(f,sep=',',header=True)
