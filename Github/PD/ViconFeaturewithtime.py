...
def extract(input_file,straight_file,turn_file,afterturn_file):
    ...
    #turn 
    '''    
    turnStartDF = df.loc[(df['spToeLX'].abs()>500) & (df['spToeLY'].abs()>500)].head(1)
    startIndex = int(turnStartDF.index.get_values())
    
    turnEndDF = df.loc[(df['spToeRX'].abs()>500) & (df['spToeRY'].abs()>500)].tail(1)
    endIndex = int(turnEndDF.index.get_values())
    '''

    turnStartDF1 = df.loc[(df['spHeelLX'].abs()>600) & (df['spHeelLY'].abs()>600)].head(1)
    turnStartDF2 = df.loc[(df['spHeelRX'].abs()>600) & (df['spHeelRY'].abs()>600)].head(1)
    startIndex = min(int(turnStartDF1.index.get_values()),int(turnStartDF2.index.get_values()))
    print startIndex
       
    turnEndDF1 = df.loc[(df['spHeelLX'].abs()>600) & (df['spHeelLY'].abs()>600)].tail(1)
    turnEndDF2 = df.loc[(df['spHeelRX'].abs()>600) & (df['spHeelRY'].abs()>600)].tail(1)
