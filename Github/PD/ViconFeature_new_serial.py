...
directory = "C:\pakinsen\Vicon\WALKER2016\PDsey160502"
...
def extract(input_file,output_file):
    ...
    #print df
    #interpolate all theNA as linear
    ...
    df['acToeLX'] = df['spToeLX'].diff(-2)/(2*timeInterval)    #-1 means the period=-1, the latter-former
    df['acToeLY'] = df['spToeLY'].diff(-2)/(2*timeInterval) 
    df['acToeLZ'] = df['spToeLZ'].diff(-2)/(2*timeInterval) 
    df['acHeelLX'] = df['spHeelLX'].diff(-2)/(2*timeInterval) 
    df['acHeelLY'] = df['spHeelLY'].diff(-2)/(2*timeInterval) 
    df['acHeelLZ'] = df['spHeelLZ'].diff(-2)/(2*timeInterval) 
    df['acHeelRX'] = df['spHeelRX'].diff(-2)/(2*timeInterval) 
    df['acHeelRY']  = df['spHeelRY'].diff(-2)/(2*timeInterval) 
    df['acHeelRZ'] = df['spHeelRZ'].diff(-2)/(2*timeInterval) 
    df['acToeRX'] = df['spToeRX'].diff(-2)/(2*timeInterval) 
    df['acToeRY'] = df['spToeRY'].diff(-2)/(2*timeInterval) 
    df['acToeRZ'] = df['spToeRZ'].diff(-2)/(2*timeInterval) 
    df['ackneeLX'] = df['spkneeLX'].diff(-2)/(2*timeInterval) 
    df['ackneeLY'] = df['spkneeLY'].diff(-2)/(2*timeInterval) 
    df['ackneeLZ'] = df['spkneeLZ'].diff(-2)/(2*timeInterval) 
    df['ackneeRX'] = df['spkneeRX'].diff(-2)/(2*timeInterval) 
    df['ackneeRY']= df['spkneeRY'].diff(-2)/(2*timeInterval) 
    df['ackneeRZ'] = df['spkneeRZ'].diff(-2)/(2*timeInterval) 
    df['acshRX'] =  df['spshRX'].diff(-2)/(2*timeInterval) 
    df['acshRY']= df['spshRY'].diff(-2)/(2*timeInterval) 
    df['acshRZ'] = df['spshRZ'].diff(-2)/(2*timeInterval) 
    df['acshLX'] = df['spshLX'].diff(-2)/(2*timeInterval) 
    df['acshLY'] = df['spshLY'].diff(-2)/(2*timeInterval) 
    df['acshLZ'] = df['spshLZ'].diff(-2)/(2*timeInterval) 
    ...
    df['acHeelR'] = df['spHeelR'].diff(-2)/(2*timeInterval) 
    ...
    df['acToeR'] = df['spToeR'].diff(-2)/(2*timeInterval) 
    ...
    df['acKneeL'] = df['spKneeL'].diff(-2)/(2*timeInterval) 
    ...
    df['acKneeR'] = df['spKneeR'].diff(-2)/(2*timeInterval) 
    ...
    df['acshR'] = df['spshR'].diff(-2)/(2*timeInterval) 
    ...
    df['acshL'] = df['spshL'].diff(-2)/(2*timeInterval) 
    ...
    df['defToeLX'] = df['spToeLX'].rolling(defWindow).apply(mad)
    df['defToeLY'] = df['spToeLY'].rolling(defWindow).apply(mad)
    df['defToeLZ'] = df['spToeLZ'].rolling(defWindow).apply(mad)
    df['defToeRX'] = df['spToeRX'].rolling(defWindow).apply(mad)
    df['defToeRY'] = df['spToeRY'].rolling(defWindow).apply(mad)
    df['defToeRZ'] = df['spToeRZ'].rolling(defWindow).apply(mad) 
    df['defHeelRX'] = df['spHeelRX'].rolling(defWindow).apply(mad)      
    df['defHeelRY'] = df['spHeelRY'].rolling(defWindow).apply(mad)      
    df['defHeelRZ'] = df['spHeelRZ'].rolling(defWindow).apply(mad)      
    df['defHeelLX'] = df['spHeelLX'].rolling(defWindow).apply(mad)      
    df['defHeelLY'] = df['spHeelLY'].rolling(defWindow).apply(mad)      
    df['defHeelLZ'] = df['spHeelLZ'].rolling(defWindow).apply(mad)      
      
    
    with open(output_file, 'a') as f:
        #df.to_csv(f,sep=',',header=True)
        df.interpolate(method='linear', axis=0).to_csv(f,sep=',',header=True)  #interplote the data 
